...
public class MPGenerator {
    ...
    // TalonTrapezoidal generates 2D array for direct use with TalonControlMode
    public enum Mode {
	...
	if (mode == Mode.SCurve) {
	    ...
	    CSVFileWriter csvFileWriter = new CSVFileWriter("SCurveProfile.csv", time_data, velocity_data,
		    distance_data, acceleration_data, finalValue, distance);
	    ...
	    CSVFileWriter csvFileWriter = new CSVFileWriter("TrapezoidalProfile.csv", time_data, velocity_data,
		    distance_data, acceleration_data, finalValue, distance);
	    csvFileWriter.writeToFile();
    ...
    /**
     * Create trapezoidal velocity profile. Doesn't account for jerk.
     * 
     * @return final time
     */
    ...
    /**
     * Create s-curve velocity profile. This reduces jerk (derivative of
     * acceleration), which means that this takes longer than a trapezoidal velocity
     * profile but increases accuracy and smoothness of motion. No check for
     * triangulation, default s-curve isn't a triangle.
     * 
     * This is deprecated. Use second order filter for s-curve instead.
     * 
     * @return final time
     */
	...
	for (time = t2 + clk; time <= t3; time += clk) {
	    ...
	    v = roundHundredThousandths(v);
	    x = p2 + (v2 * (time - t2)) + (0.5 * a2 * Math.pow((time - t2), 2))
		    + (1 / 6 * -jerk * Math.pow((time - t2), 3));
	    x = roundHundredThousandths(x);
	    addData(time, v, x, acceleration);
	}
	double v3 = v;
	...
	for (time = t6 + clk; time <= t7; time += clk) {
	    ...
	    v = roundHundredThousandths(v);
	    x = p6 + (v6 * (time - t6)) + (0.5 * a6 * Math.pow((time - t6), 2))
		    + (1 / 6 * jerk * Math.pow((time - t6), 3));
	    x = roundHundredThousandths(x);
	    addData(time, v, x, acceleration);
	}
    ...
    /**
     * Check if the maximum velocity can actually be reached in a trapezoidal motion
     * profile. If the maximum velocity can't be reached, adjust it to the final
     * velocity that it can reach with some tolerance. Maximum velocity can't be
     * reached = triangular motion profile.
     */
